...
class _AboutDoc {
  ...
  // Mirror the web's pick() helpers
  String get(String en, String jp, bool ja) => (ja && jp.isNotEmpty) ? jp : en;
  List<String> getList(List<String> en, List<String> jp, bool ja) =>
      (ja && jp.isNotEmpty) ? jp : en;
}
...
class _AboutPikuruScreenState extends State<AboutPikuruScreen>
  ...
  _AboutDoc? _data;
  bool _loading = true;
  // Language toggle — defaults to English, mirrors web app
  bool _isJa = false;
      ...
      body: CustomScrollView(
        slivers: [
          ...
          SliverAppBar(
            ...
            actions: [
              // Lang toggle — mirrors web
              Padding(
                padding: const EdgeInsets.only(right: 12),
                child: Center(
                  child: GestureDetector(
                    onTap: () => setState(() => _isJa = !_isJa),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ── Why Founded ─────────────────────────────────────
                      _SectionHeader(title: _isJa ? '設立の背景' : 'Why Pikuru Was Founded'),
                      const SizedBox(height: 12),

                      // Why body
                      if (_loading)
                        _HighlightCard(child: Column(children: [
                          _skeleton(), _skeleton(), _skeleton(width: 200),
                        ]))
                      ...
                      // ── Questions Card ──────────────────────────────────
                      _SectionHeader(title: _isJa ? '私たちが答えようとした問い' : 'The Questions We Set Out to Answer'),
                      const SizedBox(height: 12),
                      ...
                      // ── Values ──────────────────────────────────────────
                      _SectionHeader(title: _isJa ? '私たちの価値観' : 'Our Values'),
                      const SizedBox(height: 12),

                      _loading
                          ? Column(children: [1, 2, 3].map((_) => Container(
                        height: 120,
                        ...
                        children: values.asMap().entries.map((e) {
                          final v = e.value;
                          final title = (_isJa && v.titleJp.isNotEmpty) ? v.titleJp : v.title;
                          final desc  = (_isJa && v.descJp.isNotEmpty)  ? v.descJp  : v.desc;
                          return Container(
                            width: double.infinity,
                            margin: EdgeInsets.only(
                                bottom: e.key == values.length - 1 ? 0 : 12),
                            padding: const EdgeInsets.all(20),
                            decoration: BoxDecoration(
                              color: Colors.white,
                              borderRadius: BorderRadius.circular(16),
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(child: Text(entry.value,
                  style: const TextStyle(
                    fontSize: 14, color: Color(0xFF3A3A3A), height: 1.6,
                  ))),
            ],
          ),
        );
...
class _BodyText extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(text,
      style: const TextStyle(
        fontSize: 14, color: Color(0xFF3A3A3A), height: 1.7, letterSpacing: 0.1,
      ));
}