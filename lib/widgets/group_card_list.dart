...
class _GroupCardListState extends ConsumerState<GroupCardList> {
  ...
  String _getSkillLevels(Map<String, dynamic> g) {
    ...
    if (_isTruthy(g['org_skill_intermediate']))
      levels.add(_tr('Intermediate', '中級'));
  ...
  String _getSchedule(Map<String, dynamic> g) {
    final existing = (g['org_schedule'] ?? g['org_meetup_time'] ?? '')
        .toString()
        .trim();
    if (existing.isNotEmpty && existing != 'null') {
      return _isJa
  ...
  Widget build(BuildContext context) {
    ...
    final resolvedDescription = (widget.group['_resolved_description'] ?? '')
        .toString();
    final resolvedLocation = (widget.group['_resolved_location'] ?? '')
        .toString();
    final displayName = resolvedName.isNotEmpty
        ? resolvedName
        : (lang == kLangJa &&
                  (widget.group['org_name_jp'] ?? '')
                      .toString()
                      .trim()
                      .isNotEmpty
    ...
    final displayDescription = resolvedDescription.isNotEmpty
        ? resolvedDescription
        : (lang == kLangJa &&
                  (widget.group['org_description_jp'] ?? '')
                      .toString()
                      .trim()
                      .isNotEmpty
              ? widget.group['org_description_jp'].toString()
              : (widget.group['org_description'] ?? '').toString());
    final imageUrl =
